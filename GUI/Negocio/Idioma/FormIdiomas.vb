Public Class FormIdiomas
    ...
    Private _Logica As New BLL.IDIOMA
    Private _Obj As BE.IDIOMA

    Private Sub Form_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.RellenarDataGrid(Me._Logica.ConsultarTodos)
    End Sub

    Private Sub RellenarDataGrid(pListado As List(Of BE.IDIOMA))
    ...
    Private Sub GetObjSeleccionado()
        ...
        If IsNothing(Me._Obj) Then Throw New Exception("No se ha seleccionado ningún Idioma.")
    End Sub

    Private Sub AbrirDialogo()
        Dim Dialogo As New FormIdioma(Me._Obj)
        With Dialogo
            .TopMost = True
            .Show()
        End With
    ...
    Private Sub Agregar()
        Me._Obj = New BE.IDIOMA
        Me._Obj.PalabrasTraducidas = New List(Of BE.PALABRATRADUCIDA)
        Me.AbrirDialogo()
    End Sub
    ...
    Private Sub Editar()
        Try
            ...
            If Me._Obj.Predeterminado Then Throw New Exception("No se puede editar el Idioma predeterminado.")
            Me.AbrirDialogo()
    ...
    Private Sub Eliminar()
        If Not Alertador.Alertar("¿Está seguro que desea eliminar el Idioma?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then Exit Sub
        Try
            Me.GetObjSeleccionado()
            If Me._Obj.Predeterminado Then Throw New Exception("No se puede eliminar el Idioma predeterminado.")
