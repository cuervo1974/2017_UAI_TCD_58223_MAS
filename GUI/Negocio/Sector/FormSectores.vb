Public Class FormSucursales
    Implements InterfaceObservador

    Private _Logica As New BLL.SucursalLogica
    ...
    Private Sub RellenarDataGrid(pListado As List(Of Estructura.Sucursal))
    ...
    Private Sub GetObjSeleccionado()
        ...
        If IsNothing(Me._Obj) Then Throw New Exception("No se ha seleccionado ninguna Sucursal.")
    End Sub

    Private Sub AbrirDialogo()
        Dim Dialogo As New FormSucursal(Me._Obj)
        With Dialogo
            .TopMost = True
            .Show()
        End With
    ...
    Private Sub Agregar()
        Me._Obj = New Estructura.Sucursal
        Me.AbrirDialogo()
    End Sub
    ...
    Private Sub Eliminar()
        If Not Alertador.Alertar("¿Está seguro que desea eliminar la Sucursal?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then Exit Sub
        Try
            Me.GetObjSeleccionado()
