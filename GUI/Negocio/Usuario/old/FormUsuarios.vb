Public Class FormEmpleados
    ...
    Private Sub RellenarDataGrid(pListado As List(Of Estructura.Empleado))
    ...
    Private Sub GetObjSeleccionado()
        ...
        If IsNothing(Me._Obj) Then Throw New Exception("No se ha seleccionado ningún Empleado.")
    End Sub

    Private Sub AbrirDialogo()
        Dim Dialogo As New FormEmpleado(Me._Obj)
        With Dialogo
            .TopMost = True
            .Show()
        End With
    ...
    Private Sub Agregar()
        Me._Obj = New Estructura.Empleado
        Me.AbrirDialogo()
    End Sub
